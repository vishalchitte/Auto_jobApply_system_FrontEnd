...
export default function InterviewTimeTable() {
  ...
  const role = localStorage.getItem('userRole'); // "ADMIN" or "CANDIDATE"

  // Load schedules depending on role
  useEffect(() => {
    if (!role) return;
    ...
    if (role === 'ADMIN') {
      if (!adminId) return;
      url = `http://localhost:8080/api/interviews/admin/${adminId}`;
    } else {
      if (!userId) return;
      url = `http://localhost:8080/api/interviews/user/${userId}`;
    }
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 text-white">
      <h2 className="text-xl font-semibold mb-4">
        {role === 'ADMIN' ? 'All Interviews (My Users)' : 'My Interviews'}
      </h2>

      {/* Candidate can add new interview */}
      {role !== 'ADMIN' && (
        <>
          ...
          <table className="w-full border border-gray-700">
            <thead>
              <tr className="bg-gray-700">
                {role === 'ADMIN' && <th className="px-4 py-2 border border-gray-600">Candidate</th>}
                ...
                <tr key={index} className="text-center">
                  {role === 'ADMIN' && (
                    <td className="px-4 py-2 border border-gray-600">
                      {interview.userName} <br />
                      <span className="text-gray-400 text-sm">{interview.userEmail}</span>
                    </td>
                  )}
